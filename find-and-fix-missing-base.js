...
function findMissingBaseImages(shouldCreateFiles = false) {
    ...
    missingBaseFiles.forEach((item, index) => {
        ...
        if (sourceFile && shouldCreateFiles) {
            const targetDir = path.dirname(sourceFile.fullPath);
            const targetFile = path.join(targetDir, `${item.baseName}.webp`);
            filesToCreate.push({
    ...
    if (shouldCreateFiles && filesToCreate.length > 0) {
        console.log(`\nCreating ${filesToCreate.length} missing base files...`);
        ...
        console.log(`\nDone! Created ${filesToCreate.length} base files.`);
    }
...
if (args.includes('--help')) {
    console.log('Usage:');
    console.log('  bun find-and-fix-missing-base.js        # Only show missing base files');
    console.log('  bun find-and-fix-missing-base.js --create  # Create missing base files by copying from suffixed versions');
    process.exit(0);
}
